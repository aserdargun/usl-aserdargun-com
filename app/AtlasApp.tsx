...
import { PortfolioPathways } from "./components/portfolio-pathways";
...
import { emptyProgress, parseProgress, readStorage, writeStorage, type ProgressState } from "@/lib/learning-state";
import { NumericField as LabInput } from "./components/numeric-field";
...
function GlossaryTooltip({ locale, termKey, children }: { locale: Locale; termKey: string; children: React.ReactNode }) {
  ...
  return (
    <span className="glossary-wrap">
      ...
      {open && (
        <span id={tooltipId} className="glossary-pop" role="tooltip">
...
function Labs({ locale, progress, setLab }: { locale: Locale; progress: ProgressState; setLab: (key: string, value: number) => void }) {
  ...
  return (
    <section className="surface-stack">
      <div className="page-intro"><p className="eyebrow">{tr ? "KONTROLLÜ ARAÇLAR" : "CONTROLLED TOOLS"}</p><h1>{tr ? "Laboratuvar" : "Labs"}</h1><p>{tr ? "Bu araçlar planlama ve kavram doğrulama içindir. Gerçek GPU çalıştırmazlar; ölçüm alanları yalnızca senin girdiğin değerleri saklar. Veri oranları ve skor ağırlıkları değiştirilebilir öğretici varsayımlardır; doğrulanmış öneri değildir." : "These tools support planning and concept checks. They do not run a real GPU; measurement fields store only the values you enter. Dataset ratios and benchmark weights are editable teaching assumptions, not validated recommendations."}</p><EvidencePill locale={locale} level="simulation" /></div>

      <div className="lab-grid">
        <article data-content-id="lab-context-budget" className="lab-card wide"><div><span className="lab-index">01</span><h2>{tr ? "Bağlam bütçesi" : "Context budget"}</h2></div><div className="input-grid"><LabInput locale={locale} label={tr ? "Üst sınır" : "Maximum"} value={get("contextMax", 4096)} onChange={(v) => setLab("contextMax", v)} /><LabInput locale={locale} label={tr ? "Sistem" : "System"} value={get("systemTokens", 200)} onChange={(v) => setLab("systemTokens", v)} /><LabInput locale={locale} label={tr ? "Şablon" : "Template"} value={get("templateTokens", 80)} onChange={(v) => setLab("templateTokens", v)} /><LabInput locale={locale} label={tr ? "Girdi" : "Input"} value={get("inputTokens", 900)} onChange={(v) => setLab("inputTokens", v)} /><LabInput locale={locale} label="RAG" value={get("ragTokens", 1000)} onChange={(v) => setLab("ragTokens", v)} /><LabInput locale={locale} label={tr ? "Yanıt" : "Response"} value={get("responseTokens", 600)} onChange={(v) => setLab("responseTokens", v)} /></div><div className={`result ${context.fits ? "good" : "bad"}`}><strong>{context.used.toLocaleString(locale)} {tr ? "token" : "tokens"}</strong><span>{context.fits ? `${context.remaining.toLocaleString(locale)} ${tr ? "token boş" : "tokens free"}` : `${Math.abs(context.remaining).toLocaleString(locale)} ${tr ? "token taşma" : "tokens over"}`}</span></div></article>

        <article data-content-id="lab-effective-batch" className="lab-card"><div><span className="lab-index">02</span><h2>{tr ? "Toplu iş ve adımlar" : "Batch and steps"}</h2></div><div className="input-grid"><LabInput locale={locale} label={tr ? "Örnek" : "Examples"} value={get("dataset", 2000)} onChange={(v) => setLab("dataset", v)} /><LabInput locale={locale} label={tr ? "Mikro toplu iş" : "Micro batch"} value={get("micro", 1)} onChange={(v) => setLab("micro", v)} min={1} /><LabInput locale={locale} label={tr ? "Birikim" : "Accumulation"} value={get("accum", 16)} onChange={(v) => setLab("accum", v)} min={1} /><LabInput locale={locale} label="GPU" value={get("gpus", 1)} onChange={(v) => setLab("gpus", v)} min={1} /><LabInput locale={locale} label={tr ? "Dönem" : "Epoch"} value={get("epochs", 3)} onChange={(v) => setLab("epochs", v)} min={1} /></div><dl className="metrics"><div><dt>{tr ? "Etkin toplu iş" : "Effective batch"}</dt><dd>{batch.effectiveBatch}</dd></div><div><dt>{tr ? "Toplam optimizasyon adımı" : "Total optimizer steps"}</dt><dd>{batch.totalOptimizerSteps}</dd></div></dl></article>

        <article data-content-id="lab-lora-scale" className="lab-card"><div><span className="lab-index">03</span><h2>{tr ? "LoRA ölçeği" : "LoRA scale"}</h2></div><div className="input-grid"><LabInput locale={locale} label="Rank" value={rank} onChange={(v) => setLab("rank", v)} min={1} /><LabInput locale={locale} label="Alpha" value={alpha} onChange={(v) => setLab("alpha", v)} /><LabInput locale={locale} label={tr ? "Matris sayısı" : "Matrices"} value={matrices} onChange={(v) => setLab("matrices", v)} min={1} /><LabInput locale={locale} label={tr ? "Boyut" : "Dimension"} value={dimension} onChange={(v) => setLab("dimension", v)} min={1} /></div><dl className="metrics"><div><dt>LoRA α/r</dt><dd>{loraScale(rank, alpha).toFixed(3)}</dd></div><div><dt>rsLoRA α/√r</dt><dd>{loraScale(rank, alpha, true).toFixed(3)}</dd></div><div><dt>{tr ? "Adaptör parametresi" : "Adapter parameters"}</dt><dd>{loraParameterCount(rank, dimension, dimension, matrices).toLocaleString(locale)}</dd></div></dl></article>

        <article data-content-id="lab-tokenizer-measurement" className="lab-card"><div><span className="lab-index">04</span><h2>{tr ? "Türkçe token ölçümü" : "Turkish token measurement"}</h2></div><p>{tr ? "Sonuç uydurulmaz. Aynı metni iki gerçek tokenizer ile ölçüp değerleri gir." : "No result is invented. Measure the same text with two real tokenizers and enter the values."}</p><div className="input-grid"><LabInput locale={locale} label={tr ? "Taban tokenizer" : "Base tokenizer"} value={baseTokens} onChange={(v) => setLab("baseTokens", v)} /><LabInput locale={locale} label={tr ? "Aday tokenizer" : "Candidate tokenizer"} value={candidateTokens} onChange={(v) => setLab("candidateTokens", v)} /></div><div className="result"><strong>{baseTokens > 0 && candidateTokens > 0 ? `${((candidateTokens / baseTokens - 1) * 100).toFixed(1)}%` : "—"}</strong><span>{tr ? "Adayın tabana göre token farkı" : "Candidate token delta vs base"}</span></div></article>

        <article data-content-id="lab-dataset-mix" className="lab-card wide"><div><span className="lab-index">05</span><h2>{tr ? "Veri kümesi karışımı" : "Dataset mix"}</h2></div><div className="input-grid"><LabInput locale={locale} label={tr ? "Toplam örnek" : "Total examples"} value={get("mixTotal", 2000)} onChange={(v) => setLab("mixTotal", v)} /><LabInput locale={locale} label={tr ? "Standart %" : "Standard %"} max={100} value={mixPercentages.standard} onChange={(v) => setLab("mixStandard", v)} /><LabInput locale={locale} label={tr ? "Farklı ifade %" : "Paraphrase %"} max={100} value={mixPercentages.paraphrase} onChange={(v) => setLab("mixParaphrase", v)} /><LabInput locale={locale} label={tr ? "Eksik bilgi %" : "Missing info %"} max={100} value={mixPercentages.missing} onChange={(v) => setLab("mixMissing", v)} /><LabInput locale={locale} label={tr ? "Negatif %" : "Negative %"} max={100} value={mixPercentages.negative} onChange={(v) => setLab("mixNegative", v)} /><LabInput locale={locale} label={tr ? "Eskalasyon %" : "Escalation %"} max={100} value={mixPercentages.escalation} onChange={(v) => setLab("mixEscalation", v)} /></div><div className={`result ${mix.valid ? "good" : "bad"}`}><strong>{mix.sum}%</strong><span>{Object.entries(mix.counts).map(([key, value]) => `${mixLabels[key as keyof typeof mixLabels]} ${value}`).join(" · ")}</span></div></article>

        <article data-content-id="lab-weighted-benchmark" className="lab-card wide"><div><span className="lab-index">06</span><h2>{tr ? "Ağırlıklı değerlendirme" : "Weighted benchmark"}</h2></div><div className="input-grid"><LabInput locale={locale} label={tr ? "Alan 35%" : "Domain 35%"} value={get("scoreDomain", 70)} onChange={(v) => setLab("scoreDomain", v)} max={100} /><LabInput locale={locale} label={tr ? "Biçim 15%" : "Format 15%"} value={get("scoreFormat", 80)} onChange={(v) => setLab("scoreFormat", v)} max={100} /><LabInput locale={locale} label={tr ? "Güvenlik 20%" : "Safety 20%"} value={get("scoreSafety", 90)} onChange={(v) => setLab("scoreSafety", v)} max={100} /><LabInput locale={locale} label={tr ? "Belirsizlik 15%" : "Uncertainty 15%"} value={get("scoreUncertainty", 70)} onChange={(v) => setLab("scoreUncertainty", v)} max={100} /><LabInput locale={locale} label={tr ? "Korunum 15%" : "Retention 15%"} value={get("scoreRetention", 85)} onChange={(v) => setLab("scoreRetention", v)} max={100} /></div><div className="score-orb"><strong>{benchmark.toFixed(1)}</strong><span>/ 100</span></div></article>

        <article data-content-id="lab-masking-checklist" className="lab-card wide"><div><span className="lab-index">07</span><h2>{tr ? "Şablon ve maskeleme kontrolü" : "Template and masking check"}</h2></div><ol className="checklist"><li>{tr ? "Tek bir örneği gerçek tokenizer ile işle." : "Render one example with the real tokenizer."}</li><li>{tr ? "BOS, rol sınırları, asistan başlangıcı ve EOS'u görünür kıl." : "Expose BOS, role boundaries, assistant start, and EOS."}</li><li>{tr ? "Etiketlerde istem tokenlarının -100, yanıt tokenlarının etkin olduğunu doğrula." : "Verify prompt labels are -100 while response labels remain active."}</li><li>{tr ? "Çıkarımda aynı şablon ve durdurma davranışını kullan." : "Use the same template and stop behavior at inference."}</li></ol></article>

        <article data-content-id="lab-hardware-planning" className="lab-card wide"><div><span className="lab-index">08</span><h2>{tr ? "4070 Ti Super planlama rehberi" : "4070 Ti Super planning guide"}</h2></div><div className="warning"><EvidencePill locale={locale} level="simulation" /><p>{tr ? "16 GB için denenecek başlangıç ayarları (modelin sığacağı garanti edilmez): 4-bit taban, mikro toplu iş 1, bağlam 1024–2048 ve gradyan kontrol noktası. OOM sırası: toplu iş → bağlam → daha küçük model → aktarma. Bu kart gerçek bir koşunun kanıtı değildir." : "Starting settings to test on 16 GB (model fit is not guaranteed): 4-bit base, micro batch 1, context 1024–2048, gradient checkpointing. OOM order: batch → context → smaller model → offload. This card is not evidence of a real run."}</p></div></article>
      </div>
    </section>
  );
}
...
function MixedQuiz({ locale, progress, onFinish }: { locale: Locale; progress: ProgressState; onFinish: (score: number, gaps: string[]) => void }) {
  ...
  function check() {
    if (selected === null || revealed || done) return;
    setRevealed(true);
...
function DataEvaluation({ locale, progress, onFinish }: { locale: Locale; progress: ProgressState; onFinish: (score: number, gaps: string[]) => void }) {
  const tr = locale === "tr";
  return <section className="surface-stack"><div className="page-intro"><p className="eyebrow">{tr ? "VERİ → DEĞERLENDİRME → KARAR" : "DATA → BENCHMARK → DECISION"}</p><h1>{tr ? "Veri ve değerlendirme" : "Data and evaluation"}</h1><p>{tr ? "Veri sızıntısı, biçim ve güvenlik kontrollerini loss’a bakmadan önce tasarla. Aşağıdaki karışım ve kabul eşikleri bitirme projesi için öğretici örneklerdir; evrensel standart değildir. Skorlar 0–100 ölçeğinde, korunum değişimi yüzde puan cinsindedir." : "Design leakage, formatting and safety checks before inspecting loss. The mix and acceptance thresholds below are teaching examples for the capstone, not universal standards. Scores use a 0–100 scale; retention change is in percentage points."}</p></div><div className="recipe-grid">{(tr ? [["55%", "Standart", "Temel görev dağılımı"], ["10%", "Farklı ifade", "Yüzey çeşitliliği"], ["15%", "Eksik bilgi", "Belirsizlik davranışı"], ["10%", "Negatif", "Yanlış varsayıma direnç"], ["10%", "Eskalasyon", "Güvenli yönlendirme"]] : [["55%", "Standard", "Core task distribution"], ["10%", "Paraphrase", "Surface variation"], ["15%", "Missing info", "Uncertainty behavior"], ["10%", "Negative", "Resistance to false premises"], ["10%", "Escalation", "Safe redirection"]]).map(([amount, title, description]) => <article key={title}><b>{amount}</b><h3>{title}</h3><p>{description}</p></article>)}</div><div className="gate-grid"><article><h2>{tr ? "Veri kümesi kapısı" : "Dataset gate"}</h2><ul className="checklist"><li>{tr ? "Eğitim, doğrulama ve test kaynakları ayrık" : "Train/validation/test sources are disjoint"}</li><li>{tr ? "Aynı ve neredeyse aynı kayıtlar tarandı" : "Duplicate and near-duplicate scan"}</li><li>{tr ? "Kişisel veriler ve özel operasyon ayrıntıları temiz" : "PII and private operational details removed"}</li><li>{tr ? "Sohbet şablonu gerçek tokenizer ile incelendi" : "Chat template inspected with the real tokenizer"}</li></ul></article><article><h2>{tr ? "Örnek model kabul kapısı" : "Example model acceptance gate"}</h2><ul className="checklist"><li>{tr ? "Alan skoru ≥ temel model" : "Domain ≥ baseline"}</li><li>{tr ? "Biçim ≥ 95" : "Format ≥ 95"}</li><li>{tr ? "Güvenlik ≥ temel model" : "Safety ≥ baseline"}</li><li>{tr ? "Korunum düşüşü ≤ 3 yüzde puan" : "Retention drop ≤ 3 percentage points"}</li><li>{tr ? "Aynı rastgelelik tohumu ve üretim ayarları" : "Same seed and generation settings"}</li></ul></article></div><MixedQuiz locale={locale} progress={progress} onFinish={onFinish} /></section>;
}
...
function Lesson({ locale, slug, progress, toggle }: { locale: Locale; slug: string; progress: ProgressState; toggle: (id: string) => void }) {
  ...
  return (
    <article className="lesson-page" data-content-id={lesson.id}>
...
export default function AtlasApp({ locale, surface = "dashboard", slug }: { locale: Locale; surface?: Surface; slug?: string }) {
  ...
  useEffect(() => {
    ...
    const nextTheme = (savedTheme === "dark" || savedTheme === "light" ? savedTheme : null) ?? (window.matchMedia("(prefers-color-scheme: light)").matches ? "light" : "dark");
    document.documentElement.dataset.theme = nextTheme; document.documentElement.lang = locale; writeStorage(LANGUAGE_KEY, locale);
    const hydrationTimer = window.setTimeout(() => {
      setProgress(parseProgress(readStorage(PROGRESS_KEY), [...lessons[locale], ...weeks[locale]].map((item) => item.id)));
      setTheme(nextTheme);
    }, 0);
    return () => window.clearTimeout(hydrationTimer);
  }, [locale]);

  function save(next: ProgressState) { setProgress(next); setStorageUnavailable(!writeStorage(PROGRESS_KEY, JSON.stringify(next))); }
  ...
  function toggleTheme() { const next = theme === "dark" ? "light" : "dark"; setTheme(next); document.documentElement.dataset.theme = next; writeStorage(THEME_KEY, next); }

  const commitSha = process.env.NEXT_PUBLIC_COMMIT_SHA?.slice(0, 7) ?? "local";
  return <div className="site-shell"><a className="skip-link" href="#main">{t.skip}</a><header className="site-header"><Link className="brand" href={`/${locale}/`} aria-label={t.brand}><span className="brand-mark">U</span><span><b>{t.brand}</b><small>{t.brandTag}</small></span></Link><nav aria-label={locale === "tr" ? "Ana navigasyon" : "Main navigation"}><Link className={surface === "dashboard" && !slug ? "active" : ""} aria-current={surface === "dashboard" && !slug ? "page" : undefined} href={`/${locale}/`}>{t.nav.dashboard}</Link><Link className={surface === "roadmap" ? "active" : ""} aria-current={surface === "roadmap" ? "page" : undefined} href={`/${locale}/roadmap/`}>{t.nav.roadmap}</Link><Link className={slug ? "active" : ""} aria-current={slug ? "page" : undefined} href={`/${locale}/learn/models/`}>{t.nav.learn}</Link><Link className={surface === "labs" ? "active" : ""} aria-current={surface === "labs" ? "page" : undefined} href={`/${locale}/labs/`}>{t.nav.labs}</Link><Link className={surface === "evidence" ? "active" : ""} aria-current={surface === "evidence" ? "page" : undefined} href={`/${locale}/evidence/`}>{t.nav.evidence}</Link><Link className={surface === "data-evaluation" ? "active" : ""} aria-current={surface === "data-evaluation" ? "page" : undefined} href={`/${locale}/data-evaluation/`}>{t.nav.data}</Link><Link className={surface === "journey" ? "active" : ""} aria-current={surface === "journey" ? "page" : undefined} href={`/${locale}/journey/`}>{t.nav.journey}</Link><Link className={surface === "visualize" ? "active" : ""} aria-current={surface === "visualize" ? "page" : undefined} href={`/${locale}/visualize/`}>{t.nav.visualize}</Link><Link className={surface === "flashcards" ? "active" : ""} aria-current={surface === "flashcards" ? "page" : undefined} href={`/${locale}/flashcards/`}>{t.nav.flashcards}</Link></nav><div className="header-actions"><Link href={alternatePath} hrefLang={alternate} onClick={() => writeStorage(LANGUAGE_KEY, alternate)}>{t.language}</Link><button aria-label={t.theme} title={t.theme} onClick={toggleTheme}>{theme === "dark" ? "☼" : "◐"}</button></div></header><main id="main" data-content-id={slug ? undefined : `surface-${surface}`}>{storageUnavailable && <p className="warning" role="status">{locale === "tr" ? "Tarayıcı kaydı kullanılamıyor. İlerlemen yalnızca bu sayfa açıkken korunur." : "Browser storage is unavailable. Progress lasts only while this page stays open."}</p>}{slug ? <Lesson locale={locale} slug={slug} progress={progress} toggle={toggle} /> : surface === "dashboard" ? <Dashboard locale={locale} progress={progress} /> : surface === "roadmap" ? <Roadmap locale={locale} progress={progress} toggle={toggle} /> : surface === "labs" ? <Labs locale={locale} progress={progress} setLab={setLab} /> : surface === "evidence" ? <Evidence locale={locale} /> : surface === "data-evaluation" ? <DataEvaluation locale={locale} progress={progress} onFinish={finishQuiz} /> : surface === "visualize" ? <VisualizePage locale={locale} /> : surface === "flashcards" ? <FlashcardsPage locale={locale} /> : <Journey locale={locale} />}</main><footer><div><span className="brand-mark">U</span><p><b>Unsloth Studio Learning Atlas</b><br/><small>{t.localOnly}</small></p></div><div><a href={locale === "tr" ? "https://aserdargun.com/tr/" : "https://aserdargun.com/"}>aserdargun.com</a><a href="https://github.com/aserdargun/usl-aserdargun-com">GitHub</a><a href="https://creativecommons.org/licenses/by/4.0/">CC BY 4.0</a><code title="Git commit SHA">{commitSha}</code></div></footer></div>;
}
