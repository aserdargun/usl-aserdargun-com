  ...
  return (
    <section className="surface-stack">
      ...
      <div className="lab-grid">
        ...
        <article className="lab-card wide"><div><span className="lab-index">05</span><h2>{tr ? "Veri kümesi karışımı" : "Dataset mix"}</h2></div><div className="input-grid"><LabInput locale={locale} label={tr ? "Toplam örnek" : "Total examples"} value={get("mixTotal", 2000)} onChange={(v) => setLab("mixTotal", v)} /><LabInput locale={locale} label={tr ? "Standart %" : "Standard %"} max={100} value={mixPercentages.standard} onChange={(v) => setLab("mixStandard", v)} /><LabInput locale={locale} label={tr ? "Farklı ifade %" : "Paraphrase %"} max={100} value={mixPercentages.paraphrase} onChange={(v) => setLab("mixParaphrase", v)} /><LabInput locale={locale} label={tr ? "Eksik bilgi %" : "Missing info %"} max={100} value={mixPercentages.missing} onChange={(v) => setLab("mixMissing", v)} /><LabInput locale={locale} label={tr ? "Negatif %" : "Negative %"} max={100} value={mixPercentages.negative} onChange={(v) => setLab("mixNegative", v)} /><LabInput locale={locale} label={tr ? "Eskalasyon %" : "Escalation %"} max={100} value={mixPercentages.escalation} onChange={(v) => setLab("mixEscalation", v)} /></div><div className={`result ${mix.valid ? "good" : "bad"}`}><strong>{mix.sum}%</strong><span>{Object.entries(mix.counts).map(([key, value]) => `${mixLabels[key as keyof typeof mixLabels]} ${value}`).join(" · ")}</span></div></article>

        <article className="lab-card wide"><div><span className="lab-index">06</span><h2>{tr ? "Ağırlıklı değerlendirme" : "Weighted benchmark"}</h2></div><div className="input-grid"><LabInput locale={locale} label={tr ? "Alan 35%" : "Domain 35%"} value={get("scoreDomain", 70)} onChange={(v) => setLab("scoreDomain", v)} max={100} /><LabInput locale={locale} label={tr ? "Biçim 15%" : "Format 15%"} value={get("scoreFormat", 80)} onChange={(v) => setLab("scoreFormat", v)} max={100} /><LabInput locale={locale} label={tr ? "Güvenlik 20%" : "Safety 20%"} value={get("scoreSafety", 90)} onChange={(v) => setLab("scoreSafety", v)} max={100} /><LabInput locale={locale} label={tr ? "Belirsizlik 15%" : "Uncertainty 15%"} value={get("scoreUncertainty", 70)} onChange={(v) => setLab("scoreUncertainty", v)} max={100} /><LabInput locale={locale} label={tr ? "Korunum 15%" : "Retention 15%"} value={get("scoreRetention", 85)} onChange={(v) => setLab("scoreRetention", v)} max={100} /></div><div className="score-orb"><strong>{benchmark.toFixed(1)}</strong><span>/ 100</span></div></article>
